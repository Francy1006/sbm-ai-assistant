...
logger = logging.getLogger(
    "uvicorn.error.documentation_export.service"
)
...
class DocumentationExportInfrastructureError(
    RuntimeError
):
    pass
...
def _collect_project_tree(
    ...
    if (
        ...
        raise ContextValidationError(
            f"{PROJECT_TREE_FILENAME} must be "
            "a regular file"
        )
    ...
    try:
        return project_tree_path.read_text(
            encoding="utf-8"
        ).strip()
    except (
        OSError,
        ...
        raise ContextValidationError(
            f"Unable to read "
            f"{PROJECT_TREE_FILENAME} as UTF-8"
        ) from exc
...
def export_documentation(
    ...
    project_name, paths = (
        resolve_documentation_export_paths(
            ...
            documentation_root=(
                request.documentation_root
            ),
            format_context_path=(
                request.format_context_path
            ),
            system_prompt_path=(
    ...
    logger.info(
        "[DOCUMENTATION_EXPORT] discovery start "
        "project=%s",
        project_name,
    )
    ...
    logger.info(
        "[DOCUMENTATION_EXPORT] discovery complete "
        "project=%s sources=%d errors=%d",
        project_name,
        len(sources),
        len(errors),
    )
    ...
    if not sources:
        raise ContextValidationError(
            "No authorized documentation files "
            "were discovered"
        )
    ...
    if not any(
        chunks
        for _, _, chunks in chunks_by_source
    ):
        raise ContextValidationError(
            "Authorized documentation files "
            "contain no indexable content"
        )
    ...
    try:
        requested_changed_files = (
            ...
            else _collect_changed_files(
                paths.project_root
            )
        )
        changed_files = (
            _normalize_requested_paths(
                requested_changed_files
            )
        )
        ...
        logger.info(
            "[DOCUMENTATION_EXPORT] retrieval start "
            "project=%s top_k=%d",
            project_name,
        ...
        if request.retrieved_context_chunks:
            retrieved_context_chunks = (
                request.retrieved_context_chunks
            )
    ...
    try:
        ...
        logger.exception(
            "Documentation ZIP creation failed "
            "for project=%s",
            project_name,
        )
        raise (
